...
const DeleteMovieGrid = (props) => {
  ...
  useEffect(() => {
    const getList = async () => {
      ...
      response = await tmdbApi.getMoviesList(6);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.delete(
        `http://localhost:8080/api/v2/RecommendSystem/deleteAdminMovie/${movieId}`,
      ...
      <div className='add-movies-section'>
        <section>
          ...
          <form onSubmit={handleSubmit}>
            <label htmlFor='movieid'>Movie IMDBID</label>
            <input
              type='text'
              id='movieid'
              autoComplete='off'
            ...
            />
            <button disabled={!validMovieId ? true : false}>
              Delete Movie
            </button>
          </form>
